...
func parseCityGML(this js.Value, args []js.Value) any {
	...
	centroidY := (bbox.MinY + bbox.MaxY) / 2
	centroidZ := (bbox.MinZ + bbox.MaxZ) / 2

	scene := buildScene(doc, centroidX, centroidY, centroidZ)
	...
	result := map[string]any{
		...
		"bounds": map[string]any{
			"minX":      bbox.MinX - centroidX,
			"minY":      bbox.MinY - centroidY,
			"minZ":      bbox.MinZ - centroidZ,
			"maxX":      bbox.MaxX - centroidX,
			"maxY":      bbox.MaxY - centroidY,
			"maxZ":      bbox.MaxZ - centroidZ,
			"centroidX": centroidX,
			"centroidY": centroidY,
			"centroidZ": centroidZ,
			"has3D":     bbox.Has3D,
